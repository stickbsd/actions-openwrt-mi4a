...
# 修改openwrt登陆地址,把下面的192.168.168.1修改成你想要的就可以了
...
# 版本号里显示一个自己的名字（yyl build $(TZ=UTC-8 date "+%Y.%m.%d") @ 这些都是后增加的）
sed -i "s/OpenWrt /yyl build $(TZ=UTC-8 date "+%Y.%m.%d") @ OpenWrt /g" package/lean/default-settings/files/zzz-default-settings
